...
uint8_t  SCRIPT_GET8 () { uint8_t  v = mem1(scrp); scrp += 1; return v; }
uint16_t SCRIPT_GET16() { uint16_t v = mem2(scrp); scrp += 2; return v; }
uint32_t   SCRIPT_GET32() { uint32_t   v = mem4(scrp); scrp += 4; return v; }
	...
	while (game_playing) {
		...
		switch (op) {
			...
			case 0x40: { // MAP_OPTION
				uint32_t ptr = SCRIPT_GET32();
				uint32_t x1  = SCRIPT_GET16();
				uint32_t y1  = SCRIPT_GET16();
				uint32_t x2  = SCRIPT_GET16();
				uint32_t y2  = SCRIPT_GET16();
