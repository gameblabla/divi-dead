...
void fadeOutPixel(int param, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a) {
	fadeOutPixel_c(r); fadeOutPixel_c(g); fadeOutPixel_c(b); fadeOutPixel_c(a);
}

void fadeOut2Pixel(int param, uint8_t *r, uint8_t *g, uint8_t *b, uint8_t *a) {
	fadeOut2Pixel_c(r); fadeOut2Pixel_c(g); fadeOut2Pixel_c(b); fadeOut2Pixel_c(a);
}

void processSurface(SDL_Surface *s, SDL_Rect *rect, void (*func)(int, uint8_t*, uint8_t*, uint8_t*, uint8_t*), int param) {
	SDL_PixelFormat *pf = s->format;
	uint8_t r, g, b, a;
	switch (pf->BytesPerPixel) {
		case 3: case 1: return; // Not supported (8 bits nor 24 bits)
		case 2: { uint16_t *cur, *end; processSurface_iterate(); } break; // 16 bits
		case 4: { uint32_t *cur, *end; processSurface_iterate(); } break; // 32 bits
	}
}
